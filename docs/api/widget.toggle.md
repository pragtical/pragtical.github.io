...
```lua
(field) caption_label: widget.textbox
```
...
```lua
(field) toggle_bg: renderer.color
```
...
```lua
(method) widget.toggle:is_toggled()
  -> boolean
```
...
```lua
(method) widget.toggle:new(parent: widget, label: string, enable: boolean)
```
...
```lua
(method) widget.toggle:set_toggle(enabled: boolean)
```
