...
```lua
(method) widget.togglebutton:draw()
  -> boolean
```
...
```lua
(method) widget.togglebutton:is_toggled()
  -> boolean
```
...
```lua
(method) widget.togglebutton:new(parent: widget, enable?: boolean, label?: string, icon?: string)
```
...
```lua
(method) widget.togglebutton:on_click()
```
...
```lua
(method) widget.togglebutton:on_mouse_enter(...any)
```
...
```lua
(method) widget.togglebutton:on_mouse_leave(...any)
```
...
```lua
(method) widget.togglebutton:on_scale_change(new_scale: number, prev_scale: number)
```
...
```lua
(method) widget.togglebutton:set_icon(code?: string, color?: renderer.color, hover_color?: renderer.color)
```
...
@*param* `hover_color?`: [`renderer.color`](/docs/api/renderer#renderercolor) — Array of bytes that represents a color used by the rendering functions.
...
```lua
(method) widget.togglebutton:set_label(text: string)
```
...
```lua
(method) widget.togglebutton:set_toggle(enabled: boolean)
```
...
```lua
(method) widget.togglebutton:toggle()
```
...
```lua
(method) widget.togglebutton:toggle_expand(expand?: boolean)
```
...
```lua
(method) widget.togglebutton:update()
  -> boolean
```
...
```lua
(method) widget.togglebutton:update_size_position()
```
