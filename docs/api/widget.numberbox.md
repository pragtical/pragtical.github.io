...
```lua
(field) decrease_button: widget.button
```
...
```lua
(field) increase_button: widget.button
```
...
```lua
(method) widget.numberbox:decrease()
```
...
```lua
(method) widget.numberbox:get_value()
  -> number
```
...
```lua
(method) widget.numberbox:increase()
```
...
```lua
(method) widget.numberbox:mouse_pressed(increase: boolean)
```
...
```lua
(method) widget.numberbox:new(parent: widget, value: number, min?: number, max?: number, step?: number)
```
...
```lua
(method) widget.numberbox:on_scale_change(new_scale: number, prev_scale: number)
```
...
```lua
(method) widget.numberbox:set_range(min?: number, max?: number)
```
...
```lua
(method) widget.numberbox:set_step(step: number)
```
...
```lua
(method) widget.numberbox:set_value(value: number)
```
...
```lua
(method) widget.numberbox:update_size_position()
```
