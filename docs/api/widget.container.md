...
```lua
(field) default_proprties: (widget.container.properties)?
```
...
Take the whole width of parent without the padding?
...
A stretch factor proportional to other childs
...
```lua
(method) widget.container:add_child(child: widget, properties: widget.container.properties)
```
...
@*param* `properties`: [`widget.container.properties`](/docs/api/widget.container#widgetcontainerproperties) — A child widget sizing and alignment properties.
...
```lua
(method) widget.container:new(parent: widget, direction: widget.container.direction, alignment: widget.container.alignment)
```
...
@*param* `direction`: [`widget.container.direction`](/docs/api/widget.container#widgetcontainerdirection)

@*param* `alignment`: [`widget.container.alignment`](/docs/api/widget.container#widgetcontaineralignment)
...
```lua
(method) widget.container:set_child_properties(child: widget, properties?: widget.container.properties)
```
...
## set_default_child_properties

```lua
(method) widget.container:set_default_child_properties(properties?: widget.container.properties)
```
...
```lua
(method) widget.container:set_padding(padding: widget.padding)
```

Set the internal padding for the container.

@*param* `padding`: [`widget.padding`](/docs/api/widget#widgetpadding) — Represents a padding attribute.
...
```lua
(method) widget.container:set_spacing(spacing: number)
```
