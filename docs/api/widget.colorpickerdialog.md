...
```lua
(method) widget.colorpickerdialog:new(title?: string, color?: string|renderer.color)
```
...
```lua
(method) widget.colorpickerdialog:on_apply(value: renderer.color)
```

Called when the user clicks on apply

@*param* `value`: [`renderer.color`](/docs/api/renderer#renderercolor) — Array of bytes that represents a color used by the rendering functions.
...
```lua
(method) widget.colorpickerdialog:show()
```
...
```lua
(method) widget.colorpickerdialog:update_size_position()
```
