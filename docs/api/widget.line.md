...
```lua
(method) widget.line:new(parent: widget, thickness: integer, padding: number)
```
...
```lua
(method) widget.line:set_thickness(thickness: number)
```
...
```lua
(method) widget.line:set_width(width: number)
```
