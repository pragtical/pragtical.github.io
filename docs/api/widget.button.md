...
```lua
(field) hover_color: renderer.color|nil
```
...
```lua
(method) widget.button:new(parent: widget, label?: string)
```
...
```lua
(method) widget.button:on_mouse_enter(...any)
```
...
```lua
(method) widget.button:on_mouse_leave(...any)
```
...
```lua
(method) widget.button:on_scale_change(new_scale: number, prev_scale: number)
```
...
```lua
(method) widget.button:set_icon(code?: string, color?: renderer.color, hover_color?: renderer.color)
```
...
```lua
(method) widget.button:set_label(text: string)
```
...
```lua
(method) widget.button:toggle_expand(expand?: boolean)
```
...
```lua
(method) widget.button:update_size_position()
```
