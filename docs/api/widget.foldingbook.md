...
```lua
(field) panes: widget.foldingbook.pane[]
```
...
```lua
(method) widget.foldingbook:add_pane(name: string, label: string)
  -> container: widget
```
...
@*return* `container`: [`widget`](/docs/api/widget)
...
```lua
(method) widget.foldingbook:delete_pane(name: string)
  -> deleted: boolean
```
...
```lua
(method) widget.foldingbook:draw()
  -> boolean
```
...
```lua
(method) widget.foldingbook:get_pane(name: string)
  -> widget.foldingbook.pane|nil
```

@*return*: [`widget.foldingbook.pane`](/docs/api/widget.foldingbook#widgetfoldingbookpane)`|nil`
...
```lua
(method) widget.foldingbook:new(parent: widget)
```
...
```lua
(method) widget.foldingbook:on_tab_click(pane: widget.foldingbook.pane)
```

@*param* `pane`: [`widget.foldingbook.pane`](/docs/api/widget.foldingbook#widgetfoldingbookpane) — Represents a foldingbook pane
...
```lua
(method) widget.foldingbook:set_pane_icon(name: string, icon: string, color?: renderer.color, hover_color?: renderer.color)
  -> boolean
```

Set or remove the icon for the given pane.

@*param* `color?`: [`renderer.color`](/docs/api/renderer#renderercolor)`|nil` — Array of bytes that represents a color used by the rendering functions.

@*param* `hover_color?`: [`renderer.color`](/docs/api/renderer#renderercolor)`|nil` — Array of bytes that represents a color used by the rendering functions.
...
```lua
(method) widget.foldingbook:set_pane_label(name: string, label: string)
  -> boolean
```
...
```lua
(method) widget.foldingbook:toggle_pane(name: string, visible: boolean|nil)
```
...
```lua
(method) widget.foldingbook:update()
  -> boolean
```
