...
```lua
(field) active_pane: widget.notebook.pane
```
...
```lua
(field) panes: widget.notebook.pane[]
```
...
```lua
(method) widget.notebook:add_pane(name: string, label: string)
  -> container: widget
```
...
```lua
(method) widget.notebook:get_pane(name: string)
  -> widget.notebook.pane|nil
```

Search the pane for the given name and return it.

@*return*: [`widget.notebook.pane`](/docs/api/widget.notebook#widgetnotebookpane)`|nil`
...
```lua
(method) widget.notebook:new(parent: widget)
```
...
```lua
(method) widget.notebook:on_tab_click(pane: widget.notebook.pane)
```

Called when a tab is clicked.

@*param* `pane`: [`widget.notebook.pane`](/docs/api/widget.notebook#widgetnotebookpane) — Represents a notebook pane
...
```lua
(method) widget.notebook:set_pane(name: string)
  -> boolean
```
...
```lua
(method) widget.notebook:set_pane_icon(name: string, icon?: string, color?: renderer.color, hover_color?: renderer.color)
  -> boolean
```
...
```lua
(method) widget.notebook:set_pane_label(name: string, label: string)
  -> boolean
```
