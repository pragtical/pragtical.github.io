...
```lua
(field) mode: table<string, integer>
```
...
```lua
(method) widget.filepicker:add_filter(pattern: string)
```
...
```lua
(method) widget.filepicker:clear_filters()
```
...
```lua
(method) widget.filepicker:get_directory()
  -> string|nil
```
...
```lua
(method) widget.filepicker:get_filename()
  -> string|nil
```
...
```lua
(method) widget.filepicker:get_path()
  -> string|nil
```
...
```lua
(method) widget.filepicker:get_relative_path()
  -> string
```
...
```lua
(method) widget.filepicker:new(parent: widget, path?: string)
```
...
```lua
(method) widget.filepicker:set_directory(dir: string)
```
...
```lua
(method) widget.filepicker:set_filename(name: string)
```
...
```lua
(method) widget.filepicker:set_mode(mode: string|integer|`FilePicker.mode.DIRECTORY_EXISTS`|`FilePicker.mode.DIRECTORY`|`FilePicker.mode.FILE_EXISTS`...(+1))
```
...
```lua
(method) widget.filepicker:set_path(path?: string)
```
...
```lua
(method) widget.filepicker:set_size(width?: number, height?: number)
```
...
```lua
(method) widget.filepicker:show_picker()
```
...
```lua
(method) widget.filepicker:update_size_position()
```
