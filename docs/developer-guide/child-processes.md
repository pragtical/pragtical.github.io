...
- Non-blocking IO
- Able to detach processes from Pragtical
...
| Parent process  | The parent process that owns the child. This usually refers to Pragtical.
...
:::note Before v2.1.0, this table is not escaped on Windows.
:::
...
`stdin`, `stdout` and `stderr` tells Pragtical how to treat standard
